...
- iter14 基线以 **`d1e4dd0` + `scripts/run_iter14_all.py` + 同一批 CSV 数据** 为正式可复现口径
- 任何“实盘安全”逻辑（手动平仓检测、仓位对账、pending 防重入、rollback 等）都**不得直接混入策略主文件的回测路径**
- 任何影响策略时序的改动，必须先用相同脚本复现：
  - TOTAL
  - p2209
  - p2401
  - p2509
  - p2601
- 若基线偏离，优先判断为**工程实现污染了回测语义**，而不是先怀疑策略本身失效
