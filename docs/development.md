...
#### Windows / WSL 命令规范（统一适用于本文件所有 `uv run python ...`）
...
当前策略在 **LIVE** 模式下新增了 `live_warmup_days` 参数，用于控制：
- 实盘启动前，从**本地数据库**加载多少天的 1m bar 作为预热数据
- 默认值：`5`
...
1. **实盘预热优先走数据库**
   - 不优先依赖 XT datafeed
   - 原因：实时交易时，XT datafeed 可能未安装或未配置；而数据库里通常已经有采集到的历史 1m bar

2. **预热阶段不交易**
   - 当前实现是在 `on_init()` 内先完成 `load_bar(...)`
   - 预热完成后，策略才进入 `inited=True`
   - 然后用户/引擎再触发 `on_start()`，进入正式运行阶段

3. **最小预热是工程经验值，不是硬门槛**
   - 如果只看 15m MACD，约 2 个交易日已能进入较稳定区间
   - 但当前策略还依赖：5m ATR、分型、笔、中枢、2B/3B 结构
   - 因此工程上建议：
     - 最小可接受：`2`
     - 推荐默认：`5`
     - 保守稳妥：`10`
...
- **LIVE**：`load_bar(live_warmup_days, use_database=True)`
- **BACKTESTING**：保持原始回测预热口径（不受 live 逻辑影响）
