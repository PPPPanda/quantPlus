# src/qp/strategies/cta_chan_allsignals.py
"""
缠论全信号执行测试策略 - 1B/1S/2B/2S/3B/3S 全部下单.

基于 CtaChanPivotStrategy 复制，核心改动：
1. 补充 1B/1S 信号（趋势背驰买卖点）
2. 移除所有过滤器（冷却、断路器、ATR过滤、去重、15m趋势过滤）
3. 卖点(1S/2S/3S)同时开空（双向交易，增加频率）
4. 下单链路保持与主策略完全一致（buy/sell/short/cover → CTA引擎 → CTP）
...
class CtaChanAllSignalsStrategy(CtaTemplate):
    """
    缠论全信号执行测试策略.

    在所有六大买卖点均下单（1B/1S/2B/2S/3B/3S），多空双向。
    关闭所有过滤器，最大化交易频率。
    下单链路与主策略完全一致（用于排查执行问题）。
    """
