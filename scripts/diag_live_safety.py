"""Diagnostic: Compare current code vs code with live-safety features disabled.

Goal: Determine if the _pending_open/_pending_close/reconciliation changes
from the live-safety commits are causing the backtest regression.
"""
from __future__ import annotations
import sys, time, logging
from pathlib import Path
from datetime import timedelta
from copy import deepcopy

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT / "src"))

import pandas as pd
from vnpy.trader.constant import Interval
from vnpy.trader.object import TradeData, OrderData

logging.getLogger("vnpy").setLevel(logging.WARNING)
logging.getLogger("qp").setLevel(logging.WARNING)

from qp.backtest.engine import run_backtest
from qp.strategies.cta_chan_pivot import CtaChanPivotStrategy
from qp.datafeed.normalizer import normalize_1m_bars, PALM_OIL_SESSIONS


# ── Create a "baseline-like" subclass that disables all live safety features ──
class BaselineLikeStrategy(CtaChanPivotStrategy):
    """Subclass that reverts on_trade/on_order to baseline no-ops
    and removes _pending_open/_pending_close guards.
    """

    def on_bar(self, bar):
        """Override: remove position reconciliation block."""
        bar_dict = {
            'datetime': bar.datetime,
            'open': bar.open_price,
            'high': bar.high_price,
            'low': bar.low_price,
            'close': bar.close_price,
            'volume': bar.volume,
        }

        # Debug: 记录1分钟K线（仅实盘模式）
        if self._debugger and self.trading:
            self._debugger.log_kline_1m(bar_dict)

        # REMOVED: position reconciliation block

        # 1. 持仓管理：检查止损（1分钟级别）
        if self.trading and self._position != 0:
            if self._check_stop_loss_1m(bar_dict):
                self.put_event()
                return

        # 2. 信号管理：检查待触发信号
        if self.trading and self._pending_signal:
            sig_type = self._pending_signal.get('type', '')
            if (sig_type == 'Buy' and self._position == 0) or \
               (sig_type == 'CloseLong' and self._position == 1) or \
               (sig_type == 'Sell' and self._position == 0):
                self._check_entry_1m(bar_dict)

        # 3. 增量更新 15m K 线
        bar_15m = self._update_15m_bar(bar_dict)
        if bar_15m:
            self._on_15m_bar(bar_15m)

        # 4. 增量更新 5m K 线
        bar_5m = self._update_5m_bar(bar_dict)
        if bar_5m:
            self._on_5m_bar(bar_5m)

        self.put_event()

    def _check_stop_loss_1m(self, bar: dict) -> bool:
        """Override: remove _pending_close guard."""
        # REMOVED: if self._pending_close: return False
        sl_hit = False
        exit_price = 0.0

        effective_stop = self._stop_price
        if self.min_hold_bars > 0 and self._bars_since_entry <= self.min_hold_bars:
            stop_dist = abs(self._entry_price - self._stop_price)
            if self._position == 1:
                effective_stop = self._entry_price - stop_dist * 2
            elif self._position == -1:
                effective_stop = self._entry_price + stop_dist * 2

        if self._position == 1:
            if bar['low'] <= effective_stop:
                sl_hit = True
                exit_price = bar['open'] if bar['open'] < effective_stop else effective_stop
        elif self._position == -1:
            if bar['high'] >= effective_stop:
                sl_hit = True
                exit_price = bar['open'] if bar['open'] > effective_stop else effective_stop

        if sl_hit:
            # REMOVED: snapshot
            if self._position == 1:
                pnl = exit_price - self._entry_price
                self.sell(exit_price, abs(self.pos))
                self.write_log(f"多头止损: price={exit_price:.0f}, pnl={pnl:.0f}")
                action = "CLOSE_LONG"
            else:
                pnl = self._entry_price - exit_price
                self.cover(exit_price, abs(self.pos))
                self.write_log(f"空头止损: price={exit_price:.0f}, pnl={pnl:.0f}")
                action = "CLOSE_SHORT"

            if self._debugger and self.trading:
                self._debugger.log_trade(
                    action=action,
                    price=exit_price,
                    volume=self.fixed_volume,
                    position=0,
                    pnl=pnl,
                    signal_type=self._signal_type
                )

            self._position = 0
            self._trailing_active = False
            # REMOVED: self._pending_close = True
            self.signal = "止损"
            self._update_loss_streak(pnl)
            return True

        return False

    def _check_entry_1m(self, bar: dict) -> None:
        """Override: remove _pending_close guard in CloseLong."""
        signal = self._pending_signal
        if not signal:
            return

        if self._cooldown_remaining > 0 and signal['type'] == 'Buy':
            self._pending_signal = None
            self.signal = ""
            return

        if signal['type'] == 'Buy':
            if bar['low'] < signal['stop_base']:
                self._pending_signal = None
                self.signal = ""
                return
            if bar['high'] > signal['trigger_price']:
                fill_price = max(signal['trigger_price'], bar['open'])
                if fill_price > bar['high']:
                    fill_price = bar['close']
                self._open_position(1, fill_price, signal['stop_base'])

        elif signal['type'] == 'CloseLong':
            # Baseline: only check _position != 1 (no _pending_close)
            if self._position != 1:
                self._pending_signal = None
                self.signal = ""
                return
            if bar['low'] < signal['trigger_price']:
                fill_price = min(signal['trigger_price'], bar['open'])
                if fill_price < bar['low']:
                    fill_price = bar['close']
                # REMOVED: snapshot
                pnl = fill_price - self._entry_price
                self.sell(fill_price, abs(self.pos))
                self.write_log(f"3S/2S平多: price={fill_price:.0f}, pnl={pnl:.0f}")
                if self._debugger and self.trading:
                    self._debugger.log_trade(
                        action="CLOSE_LONG",
                        price=fill_price,
                        volume=self.fixed_volume,
                        position=0,
                        pnl=pnl,
                        signal_type=self._signal_type
                    )
                self._position = 0
                self._trailing_active = False
                # REMOVED: self._pending_close = True
                self._pending_signal = None
                self.signal = "平多"
                self._update_loss_streak(pnl)

        elif signal['type'] == 'Sell':
            if bar['high'] > signal['stop_base']:
                self._pending_signal = None
                self.signal = ""
                return
            if bar['low'] < signal['trigger_price']:
                fill_price = min(signal['trigger_price'], bar['open'])
                if fill_price < bar['low']:
                    fill_price = bar['close']
                self._open_position(-1, fill_price, signal['stop_base'])

    def _open_position(self, direction: int, price: float, stop_base: float) -> None:
        """Override: remove _pending_open guard and snapshot."""
        # REMOVED: _pending_open guard
        # REMOVED: snapshot
        buffer = self._calc_stop_buffer()
        # REMOVED: self._pending_open = True
        if direction == 1:
            self.buy(price, self.fixed_volume)
            self._stop_price = stop_base - buffer
            self.signal = "3B/2B买入"
            self.write_log(f"开多: price={price:.0f}, stop={self._stop_price:.0f}, buffer={buffer:.1f}")
            action = "OPEN_LONG"
        else:
            self.short(price, self.fixed_volume)
            self._stop_price = stop_base + buffer
            self.signal = "3S/2S卖出"
            self.write_log(f"开空: price={price:.0f}, stop={self._stop_price:.0f}, buffer={buffer:.1f}")
            action = "OPEN_SHORT"

        self._position = direction
        self._entry_price = price
        self._initial_stop = self._stop_price
        self._trailing_active = False
        self._bars_since_entry = 0
        self._pending_signal = None

        if direction == 1:
            if self._active_pivot is not None:
                self._active_pivot['entry_count'] = self._active_pivot.get('entry_count', 0) + 1
            self._recent_entries.append({
                'price': price,
                'bar_5m_count': self._bar_5m_count,
            })

        if self._debugger and self.trading:
            self._debugger.log_trade(
                action=action,
                price=price,
                volume=self.fixed_volume,
                position=direction,
                pnl=0,
                signal_type=self._signal_type
            )

    def on_trade(self, trade: TradeData) -> None:
        """Override: baseline no-op on_trade."""
        self.write_log(
            f"成交: {trade.direction.value} {trade.offset.value} "
            f"{trade.volume}手 @ {trade.price:.0f}"
        )
        self.sync_data()
        self.put_event()

    def on_order(self, order: OrderData) -> None:
        """Override: baseline no-op on_order."""
        pass


# ── Test contracts ──
CONTRACTS = [
    {"contract": "p2209.DCE", "csv": ROOT / "data/analyse/wind/p2209_1min_202204-202208.csv"},
    {"contract": "p2401.DCE", "csv": ROOT / "data/analyse/wind/p2401_1min_202308-202312.csv"},
]

BT_PARAMS = dict(
    interval=Interval.MINUTE,
    rate=0.0001,
    slippage=1.0,
    size=10.0,
    pricetick=2.0,
    capital=100_000.0
)

ITER14_SETTING = {
    "debug": False, "debug_enabled": False, "debug_log_console": False,
    "macd_fast": 12, "macd_slow": 26, "macd_signal": 9,
    "atr_window": 14, "atr_trailing_mult": 3.0, "atr_activate_mult": 2.5,
    "atr_entry_filter": 2.0,
    "min_bi_gap": 4, "pivot_valid_range": 6, "fixed_volume": 1,
    "cooldown_losses": 2, "cooldown_bars": 20,
    "circuit_breaker_losses": 7, "circuit_breaker_bars": 70,
    "lock_profit_atr": 0.0, "min_hold_bars": 2,
    "max_pullback_atr": 3.2, "use_bi_trailing": True,
    "stop_buffer_atr_pct": 0.02,
    "max_pivot_entries": 2, "pivot_reentry_atr": 0.6,
    "dedup_bars": 0, "dedup_atr_mult": 1.5,
    "div_mode": 1, "div_threshold": 0.39,
    "seg_enabled": False, "hist_gate": 0, "gap_extreme_atr": 0.0,
    "gap_reset_inclusion": False, "bridge_bar_enabled": False,
}


def import_csv_to_db(csv_path: Path, vt_symbol: str):
    from vnpy.trader.database import get_database
    from vnpy.trader.object import BarData
    from vnpy.trader.constant import Exchange
    from zoneinfo import ZoneInfo

    CN_TZ = ZoneInfo("Asia/Shanghai")
    db = get_database()
    symbol, exchange_str = vt_symbol.split(".")
    exchange = Exchange(exchange_str)
    db.delete_bar_data(symbol, exchange, Interval.MINUTE)

    df = pd.read_csv(csv_path, parse_dates=["datetime"])
    df = normalize_1m_bars(df, PALM_OIL_SESSIONS)
    df.sort_values("datetime", inplace=True)
    df.drop_duplicates(subset=["datetime"], keep="first", inplace=True)

    if df["datetime"].dt.tz is None:
        df["datetime"] = df["datetime"].dt.tz_localize(CN_TZ)
    else:
        df["datetime"] = df["datetime"].dt.tz_convert(CN_TZ)

    bars = []
    for _, row in df.iterrows():
        dt = row["datetime"]
        if hasattr(dt, 'to_pydatetime'):
            dt = dt.to_pydatetime()
        bar = BarData(
            symbol=symbol, exchange=exchange, datetime=dt,
            interval=Interval.MINUTE,
            volume=float(row.get("volume", 0)),
            turnover=float(row.get("turnover", 0)),
            open_interest=float(row.get("open_interest", 0)),
            open_price=float(row["open"]),
            high_price=float(row["high"]),
            low_price=float(row["low"]),
            close_price=float(row["close"]),
            gateway_name="DB",
        )
        bars.append(bar)

    db.save_bar_data(bars)
    start = df["datetime"].min().to_pydatetime()
    end = df["datetime"].max().to_pydatetime()
    return start, end, len(bars)


if __name__ == "__main__":
    print("=" * 80)
    print("DIAGNOSTIC: Live Safety Impact on Backtest")
    print("=" * 80)
    print(f"{'Contract':>8} | {'Version':>12} | {'PnL':>10} | {'Pts':>8} | {'Trades':>6}")
    print("-" * 80)

    for item in CONTRACTS:
        vt = item["contract"]
        name = vt.split(".")[0]
        csv_path = item["csv"]

        if not csv_path.exists():
            print(f"{name:>8} | SKIP - file not found: {csv_path}")
            continue

        start, end, n_bars = import_csv_to_db(csv_path, vt)

        # Run with CURRENT code
        r_curr = run_backtest(
            vt_symbol=vt,
            start=start - timedelta(days=1),
            end=end + timedelta(days=1),
            strategy_class=CtaChanPivotStrategy,
            strategy_setting=ITER14_SETTING,
            **BT_PARAMS
        )
        s_curr = r_curr.stats or {}
        pnl_curr = round(s_curr.get("total_net_pnl", 0), 1)
        pts_curr = round(pnl_curr / 10, 1)
        trades_curr = s_curr.get("total_trade_count", 0)

        # Run with BASELINE-LIKE code (live safety disabled)
        r_base = run_backtest(
            vt_symbol=vt,
            start=start - timedelta(days=1),
            end=end + timedelta(days=1),
            strategy_class=BaselineLikeStrategy,
            strategy_setting=ITER14_SETTING,
            **BT_PARAMS
        )
        s_base = r_base.stats or {}
        pnl_base = round(s_base.get("total_net_pnl", 0), 1)
        pts_base = round(pnl_base / 10, 1)
        trades_base = s_base.get("total_trade_count", 0)

        print(f"{name:>8} | {'CURRENT':>12} | {pnl_curr:>+10.0f} | {pts_curr:>+8.1f} | {trades_curr:>6}")
        print(f"{name:>8} | {'BASELINE':>12} | {pnl_base:>+10.0f} | {pts_base:>+8.1f} | {trades_base:>6}")

        # Compare trades
        if trades_curr != trades_base:
            print(f"{'':>8} | >>> DIVERGENCE: {trades_base - trades_curr} trades lost!")
            
            # Find first divergence point
            trades_curr_list = sorted(r_curr.trades, key=lambda t: t.datetime)
            trades_base_list = sorted(r_base.trades, key=lambda t: t.datetime)
            min_len = min(len(trades_curr_list), len(trades_base_list))
            for i in range(min_len):
                tc = trades_curr_list[i]
                tb = trades_base_list[i]
                if tc.datetime != tb.datetime or tc.price != tb.price or tc.direction != tb.direction:
                    print(f"{'':>8} | First divergence at trade #{i}:")
                    print(f"{'':>8} |   CURRENT:  {tc.datetime} {tc.direction.value} {tc.offset.value} @ {tc.price:.0f}")
                    print(f"{'':>8} |   BASELINE: {tb.datetime} {tb.direction.value} {tb.offset.value} @ {tb.price:.0f}")
                    break
            else:
                if len(trades_curr_list) < len(trades_base_list):
                    tb = trades_base_list[min_len]
                    print(f"{'':>8} | Current stops at trade #{min_len}, baseline has:")
                    print(f"{'':>8} |   BASELINE #{min_len}: {tb.datetime} {tb.direction.value} {tb.offset.value} @ {tb.price:.0f}")
        else:
            print(f"{'':>8} | ✅ Same trade count")
        print("-" * 80)
