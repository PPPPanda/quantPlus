"""signal_breakdown_p2601_p2509.py

统计 p2509 / p2601 在 chan_pivot(BEST) 下按信号类型(3B/3S/2B/2S)分解的 PnL/交易数/胜率。
用于判断 3S 是否为主要拖累点。

运行（Windows venv）：
  E:\clawdbot_bridge\clawdbot_workspace\work\quant\quantPlus\.venv\Scripts\python.exe \
    E:\clawdbot_bridge\clawdbot_workspace\work\quant\quantPlus\scripts\signal_breakdown_p2601_p2509.py
"""
...
class ChanPivotTesterPandas:
    ...
    def _check_signal(self, curr_bar):
        ...
        is_bull = float(curr_bar["diff_15m"]) > float(curr_bar["dea_15m"])
            ...
            if p_now["type"] == "bottom" and self.p.enable_long:
                if p_now["price"] > last_pivot["zg"] and p_last["price"] > last_pivot["zg"]:
                    if last_pivot["end_bi_idx"] >= len(self.bi_points) - self.p.pivot_valid_range and is_bull:
                        sig = "Buy"; signal_type = "3B"; trig = float(p_now["data"]["high"]); stop_base = float(p_now["price"])
                ...
                if p_now["price"] < last_pivot["zd"] and p_last["price"] < last_pivot["zd"]:
                    if last_pivot["end_bi_idx"] >= len(self.bi_points) - self.p.pivot_valid_range and is_bear:
                        sig = "Sell"; signal_type = "3S"; trig = float(p_now["data"]["low"]); stop_base = float(p_now["price"])
        ...
        if not sig and self.p.enable_2b2s:
            if p_now["type"] == "bottom" and self.p.enable_long:
                ...
                if p_now["price"] > p_prev["price"] and div and is_bull:
                    sig = "Buy"; signal_type = "2B"; trig = float(p_now["data"]["high"]); stop_base = float(p_now["price"])
            elif p_now["type"] == "top" and self.p.enable_short:
                div = float(p_now["data"]["diff"]) < float(p_prev["data"]["diff"])
                if p_now["price"] < p_prev["price"] and div and is_bear:
                    sig = "Sell"; signal_type = "2S"; trig = float(p_now["data"]["low"]); stop_base = float(p_now["price"])
        ...
        self.pending_signal = {"type": sig, "trigger_price": trig, "stop_base": stop_base, "signal_type": signal_type}
...
def main():
    files = {
        "p2509": Path(r"E:/clawdbot_bridge/clawdbot_workspace/work/quant/quantPlus/data/analyse/p2509_1min_202503-202508.csv"),
        "p2601": Path(r"E:/clawdbot_bridge/clawdbot_workspace/work/quant/quantPlus/data/analyse/p2601_1min_202507-202512.csv"),
    }

    p = StrategyParams("BEST")
    ...
    for name, fp in files.items():
        ...
        win = float((trades["pnl"] > 0).mean() * 100) if not trades.empty else 0.0
        dd = 0.0
        if not trades.empty:
        ...
        print("\n" + "=" * 80)
        print(f"{name} BEST: pnl={total_pnl:.0f}, trades={total_trades}, win={win:.2f}%, dd={dd:.0f}")
        b = breakdown(trades)
        print(b.to_string())

        if not b.empty and '3S' in b.index:
            pnl_3s = float(b.loc['3S','pnl'])
            share = pnl_3s / total_pnl * 100 if total_pnl != 0 else 0.0
            print(f"\n3S pnl={pnl_3s:.0f}，占总pnl比例={share:.1f}%（负值=拖累）")
